...
| **Tier 3** | **Containerization (Docker)** | Multi-stage Dockerfile, `.dockerignore`, and non-root security context | **COMPLETE (CI VERIFIED)** | Two-stage `python:3.11-slim` build (builder + runner) with non-root user (UID 10001). Pinned `--workers 1` for rate-limiter single-bucket and reindex-lock invariants. Container buildability directly verified live on GitHub Actions `ubuntu-latest` runner (Run `34967486747`, passing in 2m5s). |
| **Tier 3** | **Cloud Run Deployment & Fleet Ingress** | Declarative Cloud Run manifest (`cloudrun.yaml`) and keyless CI/CD deploy pipeline | **DOCUMENTED (UNPROVISIONED)** | Configured `cloudrun.yaml` (autoscale 0→3 instances, 10-req concurrency, Secret Manager bindings, startup/liveness probes) and `.github/workflows/deploy-cloud-run.yml` (OIDC auth, Artifact Registry caching). Halts at GCP auth in CI because Workload Identity Federation secrets are unprovisioned; live deployment to a reachable URL is unverified until GCP infrastructure is provisioned. |
